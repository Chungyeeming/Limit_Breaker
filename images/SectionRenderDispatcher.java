...
import net.minecraft.client.RotatingSectionStorage.Value;
...
public class SectionRenderDispatcher {
   ...
   private void runTask() {
      if (!this.closed) {
         SectionRenderDispatcher.RenderSection.SectionTask task = this.queue.poll((Vec3)this.cameraPosition.get());
   ...
   public SectionRenderDispatcher.@Nullable RenderSectionBufferSlice getRenderSectionSlice(final SectionMesh sectionMesh, final ChunkSectionLayer layer) {
      ...
      Allocation indexSlice = uberBuffers.indexBuffer.getAllocation(sectionMesh);
   ...
   public void uploadTerrainBuffersToGpu() {
      ...
      try (Uploader uploader = this.stagingBuffer.startUploading(device.createCommandEncoder())) {
   ...
   public String getStats() {
      return String.format(Locale.ROOT, "pC: %03d, aB: %02d", new Object[]{this.queue.size(), this.bufferPool.getFreeBufferCount()});
   }
   ...
   public class RenderSection implements Value {
      ...
      private final MutableBlockPos renderOrigin = new MutableBlockPos(-1, -1, -1);
      ...
      private void checkSectionMesh(final CompiledSectionMesh compiledSectionMesh) {
         ...
         for (ChunkSectionLayer layer : ChunkSectionLayer.values()) {
            SectionDraw draw = compiledSectionMesh.getSectionDraw(layer);
            if (draw != null) {
      ...
      private boolean addSectionBuffersToUberBuffer(
         ...
         try {
            SectionDraw draw = key.getSectionDraw(layer);
            if (draw != null) {
               ...
               if (vertexBuffer != null) {
                  UploadCallback<CompiledSectionMesh> callback = mesh -> this.vertexBufferUploadCallback(mesh, layer);
                  success &= sectionBuffers.vertexBuffer.addAllocation(key, callback, vertexBuffer);
               }

               if (indexBuffer != null) {
                  boolean sortedIndexBuffer = vertexBuffer == null;
                  UploadCallback<CompiledSectionMesh> callback = mesh -> this.indexBufferUploadCallback(mesh, layer, sortedIndexBuffer);
      ...
      private class CompileTask extends SectionRenderDispatcher.RenderSection.SectionTask {
         ...
         public SectionRenderDispatcher.RenderSection.SectionTask.SectionTaskResult doTask(final SectionBufferBuilderPack buffers) {
            ...
            if (results.renderedLayers.isEmpty()) {
               SectionMesh oldMesh = RenderSection.this.setSectionMesh(results.blockEntities.isEmpty() ? CompiledSectionMesh.EMPTY : compiledSectionMesh);
               SectionRenderDispatcher.this.copyLock.lock();
